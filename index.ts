...
async function getQuotePrice(mint: PublicKey): Promise<number | null> {
  try {
    ...
    if (DEBUG_TX) dbg(`quote fail for ${mint.toBase58().slice(0, 8)}: ${String(e).slice(0, 80)}`);
    return null;
  }
}
...
async function handleAlphaTransaction(sig: string, signer: string, label: 'active' | 'candidate') {
  ...
  if (!tx || !tx.meta) {
    if (DEBUG_TX) dbg(`ignored tx ${sig.slice(0, 8)}: no meta`);
    return;
  }
  ...
  if (mints.length === 0) {
    if (DEBUG_TX) dbg(`ignored tx ${sig.slice(0, 8)}: no post-token increase (no new mints)`);
    return;
  }

  for (const m of mints) {
    if (DEBUG_TX) dbg(`considering mint ${m.slice(0, 8)} from ${signer.slice(0, 8)} (${label})`);
    
    if (seenMints.has(m)) {
      if (DEBUG_TX) dbg(`ignored mint ${m.slice(0, 8)}: already seen`);
      continue;
    }
    seenMints.add(m);
    ...
    if (label === 'candidate') {
      ...
      if (DEBUG_TX) dbg(`candidate ${signer.slice(0, 8)} scored on ${m.slice(0, 8)} (no buy by design)`);
    ...
    try {
      ...
      if (!report.ok) {
        if (DEBUG_TX) dbg(`skip buy ${m.slice(0, 8)}: checks failed -> ${report.reasons.join('|')}`);
